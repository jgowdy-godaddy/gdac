...
class AgenticCoderApp(App):
    ...
    BINDINGS = [
        ...
        Binding("ctrl+r", "search_history", "Search history", show=False),
    ...
    def __init__(
        self, input_callback: Optional[Callable[[str], Any]] = None, session=None
    ):
        super().__init__()
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            yield Static(
                "[bold]GoDaddy Agentic Coder (gdac)[/bold] - AI-powered coding assistant",
                id="header",
            )
...
[/]"""
        ...
        if self.session:
            ...
            if hasattr(self.session, "is_resumed") and self.session.is_resumed:
                info_lines.append(
                    f"[dim]Session: Resumed ({self.session.turn_count} previous turns)[/dim]"
                )
            else:
                info_lines.append("[dim]Session: New[/dim]")

            # Context info
            if hasattr(self.session, "agent") and hasattr(
                self.session.agent, "context_window"
            ):
                context_info = f"Context: {self.session.agent.context_window:,} tokens"
                if hasattr(self.session, "history") and self.session.history:
                    try:
                        current_tokens = self.session.agent._count_tokens(
                            self.session.history
                        )
                        usage_pct = (
                            current_tokens / self.session.agent.context_window
                        ) * 100
            ...
            if hasattr(self.session, "model_info"):
                ...
                if info.get("type") == "remote":
                    info_lines.append(
                        f"[dim]Model: {info.get('name', 'Unknown')} ({info.get('provider', 'Unknown')})[/dim]"
                    )
                    info_lines.append(
                        f"[dim]API: {info.get('base_url', 'Unknown')}[/dim]"
                    )
                    info_lines.append(
                        f"[dim]Auth: API key from {info.get('key_source', 'Unknown')}[/dim]"
                    )
                else:
        ...
        output.write(Panel.fit(content, border_style="cyan"))
    ...
    def on_key(self, event: events.Key) -> None:
        """Handle key events for history navigation and search."""
        ...
        if self._search_mode:
            if event.key == "escape":
                self._exit_search_mode(input_widget, accept=False)
                event.stop()
                return
            elif event.key == "enter":
                self._exit_search_mode(input_widget, accept=True)
                event.stop()
                return
            elif event.key == "ctrl+r":
                # Continue search with next match
        ...
        # Regular history navigation (only when not in search mode)
    ...
    def _update_status(self) -> None:
        ...
        if self.session:
            ...
            if hasattr(self.session, "agent"):
                if hasattr(self.session.agent, "context_window"):
                    try:
                        if hasattr(self.session, "history") and self.session.history:
                            ...
                            if history_text:
                                ...
                                if char_count > 1000000:  # 1MB+ of text is suspicious
                                    debug_logger.error(
                                        f"EXTREMELY LARGE HISTORY DETECTED: {char_count:,} characters"
                                    )
                                    debug_logger.error(
                                        "This suggests a bug in history management - using fallback"
                                    )
                                    # Use a very conservative estimate and cap it
                                    tokens = min(
                                        50000, self.session.agent.context_window // 2
                                    )
                                    usage_pct = (
                                        tokens / self.session.agent.context_window
                                    ) * 100
                                    status_parts.append(
                                        f"Context: ~{tokens:,}/{self.session.agent.context_window:,} ({usage_pct:.1f}%) - CORRUPTED"
                                    )
                                else:
                                    debug_logger.info(
                                        f"=== Context Calculation Debug ==="
                                    )
                                    debug_logger.info(
                                        f"History type: {type(self.session.history)}"
                                    )
                                    debug_logger.info(
                                        f"History character count: {char_count:,}"
                                    )
                                    debug_logger.info(
                                        f"History preview (first 200 chars): {history_text[:200]}"
                                    )
                                    debug_logger.info(
                                        f"History preview (last 200 chars): {history_text[-200:]}"
                                    )
                                    ...
                                    try:
                                        ...
                                        if (
                                            hasattr(
                                                self.session.agent, "anthropic_client"
                                            )
                                            and self.session.agent.anthropic_client
                                        ):
                                            debug_logger.info(
                                                "Using Anthropic client tokenizer"
                                            )
                                            try:
                                                    ...
                                                    messages=[
                                                        {
                                                            ...
                                                            "content": history_text[
                                                                :100000
                                                            ],  # Limit to avoid huge requests
                                                        }
                                                    ],
                                                )
                                                tokens = token_count.input_tokens
                                                debug_logger.info(
                                                    f"Anthropic tokenizer result: {tokens:,} tokens"
                                                )
                                            except Exception as e:
                                                debug_logger.error(
                                                    f"Anthropic beta tokenizer failed: {e}"
                                                )
                                                # Fallback to existing method
                                                tokens = (
                                                    self.session.agent._count_tokens(
                                                        history_text
                                                    )
                                                )
                                                debug_logger.info(
                                                    f"Fallback _count_tokens result: {tokens:,} tokens"
                                                )
                                        else:
                                            debug_logger.info(
                                                "Using fallback _count_tokens method"
                                            )
                                            # Fallback to the existing method
                                            tokens = self.session.agent._count_tokens(
                                                history_text
                                            )
                                            debug_logger.info(
                                                f"Fallback tokenizer result: {tokens:,} tokens"
                                            )
                                    except Exception as e:
                                        debug_logger.error(
                                        ...
                                        debug_logger.info(
                                            f"Using character estimation: {tokens:,} tokens"
                                        )

                                    # Log if tokens seem excessive
                                    if tokens > 10000:
                                        debug_logger.warning(
                                            f"HIGH TOKEN COUNT: {tokens:,} tokens from {char_count:,} characters"
                                        )
                                        debug_logger.warning(
                                            f"Ratio: {char_count/tokens:.2f} chars per token"
                                        )
                                    ...
                                    if original_tokens != tokens:
                                        debug_logger.warning(
                                            f"Token count capped from {original_tokens:,} to {tokens:,}"
                                        )
                                    ...
                                    status_parts.append(
                                        f"Context: {tokens:,}/{self.session.agent.context_window:,} ({usage_pct:.1f}%)"
                                    )
                                    debug_logger.info(
                                        f"Final display: {tokens:,}/{self.session.agent.context_window:,} ({usage_pct:.1f}%)"
                                    )
                                    debug_logger.info(f"=== End Context Debug ===\n")
                            else:
                                status_parts.append(
                                    f"Context: 0/{self.session.agent.context_window:,} (0.0%)"
                                )
                        else:
                            status_parts.append(
                                f"Context: 0/{self.session.agent.context_window:,} (0.0%)"
                        ...
                        status_parts.append(
                            f"Context: 0/{self.session.agent.context_window:,} tokens"
                        )
            ...
            if hasattr(self.session, "streaming"):
                status_parts.append(
                    f"Streaming: {'ON' if self.session.streaming else 'OFF'}"
                )
        ...
        try:
            ...
            if os.path.exists(history_file):
                with open(history_file, "r", encoding="utf-8") as f:
                    ...
                    self.history_index = len(self.command_history)
                debug_logger.info(
                    f"Loaded {len(self.command_history)} commands from history"
                )
        except Exception as e:
    ...
    def action_search_history(self) -> None:
        """Start fuzzy history search mode (Ctrl+R)."""
        input_widget = self.query_one("#input", Input)
        self._start_search_mode(input_widget)

    def _start_search_mode(self, input_widget: Input) -> None:
        """Enter search mode."""
        self._search_mode = True
        self._search_query = ""
        self._search_matches = []
        self._search_index = -1
        self._original_input = input_widget.value
        input_widget.value = ""
        input_widget.placeholder = "(reverse-i-search)`':"
        debug_logger.info("Entered search mode")

    def _exit_search_mode(self, input_widget: Input, accept: bool = False) -> None:
        """Exit search mode."""
        self._search_mode = False

        if accept and self._search_matches and self._search_index >= 0:
            # Accept current search result
            selected_command = self._search_matches[self._search_index][1]
            input_widget.value = selected_command
            input_widget.cursor_position = len(selected_command)
            debug_logger.info(f"Search accepted: {selected_command}")
        else:
            # Cancel search, restore original input
            input_widget.value = self._original_input
            input_widget.cursor_position = len(self._original_input)
            debug_logger.info("Search cancelled")

        # Reset search state
        input_widget.placeholder = (
            "Type a command or request... (/ for commands, /help for help)"
        )
        self._search_query = ""
        self._search_matches = []
        self._search_index = -1
    ...
    def _update_search(self) -> None:
        ...
        for i, cmd in enumerate(reversed(self.command_history)):
            if query_lower in cmd.lower():
                original_index = len(self.command_history) - 1 - i
        ...
        if matches:
            ...
            current_match = matches[0][1]
            input_widget.value = current_match
            input_widget.cursor_position = len(current_match)

            # Update placeholder to show search progress
            match_count = len(matches)
            input_widget.placeholder = f"(reverse-i-search)`{self._search_query}': {current_match} ({self._search_index + 1}/{match_count})"
            debug_logger.info(
                f"Search updated: '{self._search_query}' found {match_count} matches"
            )
        else:
            self._search_index = -1
            input_widget.value = ""
            input_widget.placeholder = (
                f"(failed reverse-i-search)`{self._search_query}':"
            )
            debug_logger.info(
                f"Search updated: '{self._search_query}' no matches found"
            )
    ...
    def _next_search_match(self) -> None:
        ...
        current_match = self._search_matches[self._search_index][1]

        input_widget = self.query_one("#input", Input)
        input_widget.value = current_match
        input_widget.cursor_position = len(current_match)

        # Update placeholder to show current match
        match_count = len(self._search_matches)
        input_widget.placeholder = f"(reverse-i-search)`{self._search_query}': {current_match} ({self._search_index + 1}/{match_count})"
        debug_logger.info(f"Next search match: {self._search_index + 1}/{match_count}")
...
class TextualUI:
    ...
    def start(self, input_callback: Callable[[str], None]) -> None:
        ...
        self.app = AgenticCoderApp(
            input_callback=self.input_callback, session=self.session
        )
